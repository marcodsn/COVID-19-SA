...
# NOTE: In a real knitting scenario, ensure the CSV path is correct.
# For this presentation, we assume the file exists as per the source report.
# If the file is missing, these chunks might need eval=FALSE.
```
...
```{r network_visualization, fig.height=5, fig.width=10}
...
```{r temporal_viz, fig.height=4.5, echo=FALSE}
...
**Insight:** While Lombardy had the most *cases*, **Roma and Milano** were the network *hubs*. They acted as the "pulse" of the national trend, synchronizing the acceleration/deceleration across the country.
...
**Result:** The network is **robust to random failure** (Blue) but **extremely fragile to targeted attacks** (Red). Removing just top 20 hubs shatters the network.
...
2.  **Lockdown Effect:** Almost paradoxically, the strict lockdown increased synchronization (higher density), while reopening fragmented the trends.
...
4.  **Policy Implication:** Future containment should prioritize "fire-breaking" major transport hubs. Disconnecting these specific nodes is highly efficient at breaking the national chain of contagion.